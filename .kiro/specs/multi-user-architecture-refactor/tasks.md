...
- [ ] 4.1 创建用户缓存管理器
  - [ ] 4.1.1 创建 `kernel/cache/user_cache.go` 文件
  - [ ] 4.1.2 定义 `UserCache` 结构体
  - [ ] 4.1.3 实现 `GetCache(ctx *WorkspaceContext)` 方法
  - [ ] 4.1.4 实现缓存隔离机制
  - [ ] 4.1.5 实现缓存清理机制

- [ ] 4.2 重构缓存访问
  - [ ] 4.2.1 修改所有缓存访问代码
  - [ ] 4.2.2 添加 `ctx *WorkspaceContext` 参数
  - [ ] 4.2.3 使用用户缓存管理器
  - [ ] 4.2.4 编译测试
  - [ ] 4.2.5 功能测试
...
- [ ] 5.1 移除 workspace 切换逻辑
  - [ ] 5.1.1 删除 `CheckWebAuth` 中的 workspace 切换代码
  - [ ] 5.1.2 删除 `workspaceMutex` 互斥锁
  - [ ] 5.1.3 删除 `defer` 恢复逻辑
  - [ ] 5.1.4 编译测试
